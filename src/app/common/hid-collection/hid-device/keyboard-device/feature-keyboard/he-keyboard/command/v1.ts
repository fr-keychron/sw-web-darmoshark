...
import {HidDeviceEventType, IDks, IHeConf, IProfileBuffer} from "../../../types";

export class HECommandV1 implements IHeCommand {
		...
		return new Observable(s => {
			...
			const subj = this.keyboard.report$
				.pipe(
					...
					map(v => {
						const [, , current, total, size1, size2] = v;
						return {
							current, total, size: size2 << 8 | size1
						}
					}),
			...
			.pipe(
				...
				map(v => {
					...
					return {
						...
						dks: dsk,
						analog: analog,
						analogKey: byteArr[byteArr.length - 3] === 1,
						toggle: toggle,
						profileNames: ['', '', ''],
						size: profileSize,
	...
	public getKeyDistance(row: number, col: number): Observable<{ distance: number, row?: number, col?: number }> {
		return new Observable(s => {
			...
			const subj = this.keyboard.report$
				.pipe(
					filter(v => v[0] === EKeyboardCommand.KC_HE && v[1] === EHECommand.AMC_GET_REALTIME_TRAVEL),
					map(v => v[2])
				).subscribe(v => {
					s.next({distance: v})
					subj.unsubscribe()
	...
	public enableJoyKeyboard(v: number): Observable<Result> {
		return new Observable<Result>(s => {
			const r = Result.build()
			const buf = BaseKeyboard.Buffer()
			buf[0] = EKeyboardCommand.KC_HE;
