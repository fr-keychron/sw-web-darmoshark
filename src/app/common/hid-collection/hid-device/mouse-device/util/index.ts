...
export const getMouseButtonInfo = (value:number, bufferArr?: number[]) => {
	if(bufferArr[0] === 9){
		return  {type: 'mouseMacro'}
	}
	if(bufferArr[1] &&  bufferArr[1] <= 8){
	...
	const data =  EDmsMouseBtnDefault.find(i => i.value === value);
	if(data){
		return  data
	}
	if(value === 504 && bufferArr[3] === 1){
		return {key: 'upScroll', value: 501, mouseKey: 6}
	}
	if(value === 504 && bufferArr[3] === 255){
		return {key: 'downScroll', value: 501, mouseKey: 7}
	}
}
